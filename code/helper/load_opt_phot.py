...
def load_18cow_phot(filename = "../data/data_cow/phot/Perley2019_tab4.dat"):
    f = open(filename)
    lines = f.readlines()
    f.close()
...
def load_ztf_lc(dobin = True, mjd_max = 59060):
    ...
    # Yuhan: I think there is no point in this....
    """
    fcqfs = np.unique(tb["fcqfid"].values)
    for i in range(len(fcqfs)):
        fcqfid = fcqfs[i]
        dfnow = tb[tb.fcqfid==fcqfid]
        dfnow = dfnow[dfnow.mjd < 58800]
        data = dfnow['Fpsf'].values
        data_unc = dfnow['Fpsf_unc'].values
        ndata = len(data)
        ...
        if ndata<30:
            ...
            print ('   weighted mean:', dtweightedmean)
...
def load_atlas_lc(dobin = True, mjd_max = 59060):
    ...
    ax.errorbar(tb['mjd'].values[ixc], tb['Fratio'].values[ixc], tb['Fratio_unc'].values[ixc], color = colorc, zorder = 1)
    ax.errorbar(tb['mjd'].values[ixo], tb['Fratio'].values[ixo], tb['Fratio_unc'].values[ixo], color = coloro, zorder = 1)
    cbar = ax.scatter(tb['mjd'].values, tb['Fratio'].values, c = tb["maj"].values-tb["min"].values, zorder = 2)
...
def load_koala_lc(SNT = 3):
    ...
    # Yuhan: I think there is no point in this....
    """
    plt.errorbar(tb["mjd"], tb["Fratio"], tb["Fratio_unc"])
    
    fcqfs = np.unique(tb["fcqfid"].values)
    for i in range(len(fcqfs)):
        fcqfid = fcqfs[i]
        dfnow = tb[tb.fcqfid==fcqfid]
        dfnow = dfnow[dfnow.mjd < 58360]
        data = dfnow['Fpsf'].values
        data_unc = dfnow['Fpsf_unc'].values
        ndata = len(data)
        print ("fcqfid = %d, Ndata = %d"%(np.int(fcqfid), ndata))
...
def load_98bw_lc():
    tb = pd.read_csv("../data/data_98bw/98bw_phot.csv")
    
    tb = tb.drop(columns = ["upperlimit", "e_magnitude", "telescope", "instrument", "source"])
